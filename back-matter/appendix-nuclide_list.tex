...
The core temperature rise in the MSRE during steady state operation was found to 22.2 $^{\circ}$C with an inlet temperature of 632.2 $^{\circ}$C and outlet temperature of 654.4 $^{\circ}$C \cite{engel1962}. The bulk mean fuel temperature can be calculated by \cite{engel1962}:

\begin{equation}
	\overline{T}_{f} = T_{\text{in}} + 0.46(T_{\text{out}} - T_{\text{in}}),
\end{equation}

\noindent where temperature is in Fahrenheit. For the lump depletion cases, the bulk mean temperature is calculated to be 1203.06 $^{\circ}$F or 650.6 $^{\circ}$C. 
